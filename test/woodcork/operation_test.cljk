(ns woodcork.operation-test
  "Smoke tests for the compiled WoodCorkOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `woodcork.governor-contract-test`; the Store contract in
  `woodcork.store-contract-test`."
...
(deftest test-actor-builds
  (testing "WoodCorkOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "equip-001" :severity :moderate :description "equipment-safety concern"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))

(deftest test-shipment-coordination-proposal
  (testing "Shipment coordination proposal is submitted and (when within unit count) escalates for approval"
    (let [s (-> (store/mem-store) (store/sample-data!))
          actor (op/build s)
