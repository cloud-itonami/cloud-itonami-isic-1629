...
(defn shipment-unit-count-exceeded?
  ...
  (let [capacity (:unit-count batch)
    ...
    (and (number? capacity)
         ...
         (> (Math/round (* 10000 (+ (double so-far) (double new-unit-count))))
            (Math/round (* 10000 (double capacity))))))) 

(defn shipment-unit-count-exceeded-checkable?
  "Can `batch`'s headroom actually be computed for `new-unit-count`?

  `shipment-unit-count-exceeded?` answers only `over` / `not over`, and its
  ...
  (boolean (and (map? batch)
                (number? (:unit-count batch))
                (number? (:shipped-unit-count batch 0.0))
                (number? new-unit-count))))
