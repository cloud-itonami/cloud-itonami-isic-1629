(ns woodcork.operation
  "WoodCorkOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (WoodCorkStrawAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Wood, Cork &
  Straw Products Shop Plant Operations Governor (:govern) and the
...
(defn build
  "Compiles a WoodCorkOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `woodcork.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; WoodCorkStrawAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Wood, Cork & Straw Products Shop Plant Operations Governor -- independent censor.
      (g/add-node :govern
