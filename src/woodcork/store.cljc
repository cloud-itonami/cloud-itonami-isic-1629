(ns woodcork.store
  "SSoT for the wood/cork/straw-products-shop plant-operations
  ...
  Scope note: like the closest domain analogs (`cloud-itonami-isic-
  1621`/`1622`/`1623`), this build ships a single `MemStore` backend
  ...
  is self-contained (no external wood/cork/straw-products capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-spec/unit-count/output-quality
                             record (wooden tools/handles, cork
                             stoppers, wicker/basketry items).
                             ...
                             production ledger; `:shipped-unit-count`
                             tracks the batch's own cumulative-shipped
                             ground truth.
    - `equipment`         -- a cutting/molding/weaving-equipment
                             unit's own record (handle-turning lathe,
                             cork-stopper molding press, wicker-
                             weaving loom). `:verified?`/`:registered?`
                             track whether it has actually been
                             inspected/commissioned and is on file --
                             the same ground-truth discipline as
                             `batches`.
    - `maintenance`       -- a scheduled cutting/molding/weaving-
    ...
    - `shipments`         -- a proposed outbound wood/cork/straw-
                             products shipment DRAFT
                             (`woodcork.registry`'s
                             `register-shipment`).
  ...
  independently-recomputed unit count, approved by whom, which safety
  concern was flagged' is always a query over an immutable log -- the
  audit trail a shop owner or wood/cork/straw-products buyer trusting
  this coordinator needs."
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (woodcork.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (woodcork.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-spec :handle-standard :product-type :handle
                :unit-count 500 :output-quality-percent 96.0
                :verified? true :registered? true
                :shipped-unit-count 100
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-spec :cork-stopper-standard-bore :product-type :cork-stopper
                :unit-count 80 :output-quality-percent 92.0
                :verified? true :registered? true
                :shipped-unit-count 75
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-spec :wicker-small-weave :product-type :basketry
                :unit-count 60 :output-quality-percent 89.0
                :verified? false :registered? false
                :shipped-unit-count 0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :handle-turning-lathe
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :wicker-weaving-loom
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `woodcork.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `woodcork.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
      ...
      (let [shipment-id (first path)
        ...
        (swap! a (fn [state]
                   (-> state
                       ...
                       (update-in [:batches batch-id :shipped-unit-count]
                                  (fn [prev]
                                    (+ (long (or prev 0))
                                       (long (or (:unit-count value) 0))))))))
        result)
...
(defn sample-data!
  ...
  shipped (a small new shipment blows through its own logged unit
  count -- HARD hold), one UNVERIFIED/unregistered batch (blocks any
  shipment coordinated against it); one verified+registered handle-
  turning-lathe unit (schedulable for maintenance), one
  UNVERIFIED/unregistered wicker-weaving-loom unit (blocks any
  maintenance scheduling against it) -- so the actor + demo + tests
