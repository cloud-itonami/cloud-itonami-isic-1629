(ns woodcork.advisor
  "WoodCorkStrawAdvisor -- the *contained intelligence node* for the
  wood/cork/straw-products-shop plant-operations coordination actor.

  It normalizes production-batch patches (product-spec/unit-count/
  output-quality for wooden tools/handles, cork stoppers, and wicker/
  basketry items), drafts a cutting/molding/weaving-equipment
  maintenance scheduling proposal against a piece of equipment, drafts
  a safety-concern flag (materials-safety / equipment-safety), and
  drafts an outbound wood/cork/straw-products shipment coordination
  proposal against a production batch. CRITICAL: it is a smart-but-
  untrusted advisor. It returns a *proposal* (with a rationale + the
  fields it cited), never a committed record and NEVER a real
  cutting/molding/weaving-equipment actuation, cutting/molding/
  weaving-line-run execution, or freight dispatch. Every output is
  censored downstream by `woodcork.governor` before anything touches
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; molding/weaving-equipment-control or
                                 ; cutting/molding-line-finalize effect
  ...
  contract every caller of this actor agrees to) -- `woodcork.governor`
...
(defn- log-production-batch
  ...
  validates the patch; it does not invent the batch's product spec,
  unit count, output quality or verification status. High confidence,
...
(defn- flag-safety-concern
  ...
  `woodcork.phase`: no phase ever adds this op to a phase's `:auto`
  set; `woodcork.governor` also always escalates on `:coordination/
...
(defn- coordinate-shipment
  "Draft an outbound wood/cork/straw-products shipment coordination
  proposal against a production batch. The advisor passes through the
  caller's own claimed unit count -- it does NOT invent one, and
  `woodcork.governor` NEVER trusts it: it independently recomputes
  ...
  (let [batch-id (:batch-id value)
        ...
        over-count? (and b (registry/shipment-unit-count-exceeded?
                            b (:unit-count value)))]
    {:summary    (str subject " 向け出荷調整提案 ("
     ...
     :value      value
     :stake      nil
     :confidence (if (and ready? (not over-count?)) 0.9 0.3)}))
...
(def ^:private system-prompt
  (str "あなたは木製品・コルク製品・わら/植物繊維編組製品(木製工具/柄・"
       "コルク栓・かご/バスケット細工)工場プラント運用コーディネーターの"
       ...
       "切断・成形・製織ラインの稼働確定(finalize)や設備の直接操作を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `woodcork.governor`
  escalates/holds -- an LLM hiccup can never auto-schedule
  maintenance, auto-flag a concern, or auto-coordinate a shipment."
  [content]
