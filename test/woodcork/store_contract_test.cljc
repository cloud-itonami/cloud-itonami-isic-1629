(ns woodcork.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `woodcork.store` ns docstring for why a second (Datomic-backed)
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:product-spec :cork-stopper-standard-bore}})
    (is (= :cork-stopper-standard-bore (:product-spec (store/batch s "batch-001"))))
...
(deftest safety-concern-flag-appends
  (let [s (seeded)]
    ...
    (store/commit-record! s {:effect :safety-concern/flag :path ["concern-2"]
                             :value {:equipment-id "equip-002" :severity :high}})
    (is (= 2 (count (store/safety-concerns s))) "append-only")))

(deftest shipment-propose-commits-and-advances-sequence-and-batch-unit-count
  (let [s (seeded)]
    ...
    (is (= 150 (:shipped-unit-count (store/batch s "batch-001")))
