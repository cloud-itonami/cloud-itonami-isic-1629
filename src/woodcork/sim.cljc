(ns woodcork.sim
  ...
  molding-line run (permanently blocked, no override), a
  double-schedule of the same maintenance window, a production-batch
  patch with a fabricated product spec, and a production-batch patch
  with an implausible output-quality reading.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on equip-001 (verified, registered, handle-turning lathe -- escalates, approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on equip-002 (UNVERIFIED/unregistered wicker-weaving loom -> HARD hold) ==")
    ...
    (println "== log-production-batch batch-001 with a fabricated product spec -> HARD hold ==")
