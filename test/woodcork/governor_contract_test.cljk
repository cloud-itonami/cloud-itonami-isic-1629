(ns woodcork.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control cutting/molding/weaving equipment
  directly... does NOT authorize or execute maintenance/cutting-
  molding-line operations') implemented faithfully. The single
  invariant under test:

    WoodCorkStrawAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Wood, Cork & Straw Products
    Shop Plant Operations Governor would reject; `:schedule-
    maintenance`/`:flag-safety-concern`/`:coordinate-shipment` NEVER
    auto-commit at any phase; `:log-production-batch` (no physical/
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-spec :handle-standard}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :handle-standard (:product-spec (store/batch db "batch-001"))) "SSoT actually updated")
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t2"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                     :value {:equipment-id "equip-001" :maintenance-type :lathe-blade-and-chuck-service
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-lathe :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t5"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-2"
                     :value {:equipment-id "equip-002" :maintenance-type :loom-tension-calibration
                             :scheduled-date "2026-08-01" :finalize? false}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (not= :interrupted (:status res)))
...
(deftest cutting-molding-line-finalize-is-held-and-permanently-blocked
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t8"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-3"
                     :value {:equipment-id "equip-001" :maintenance-type :cutting-molding-line-run-finalize
                             :scheduled-date "2026-09-01" :finalize? true}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:cutting-molding-line-finalize-blocked} (-> (store/ledger db) last :basis)))
      (is (empty? (store/maintenance-history db))))))
...
(deftest invalid-product-spec-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-spec :custom-oversize-novelty-item}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-spec} (-> (store/ledger db) last :basis)))
    (is (not= :custom-oversize-novelty-item (:product-spec (store/batch db "batch-001"))) "fabricated product spec never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "equip-001" :severity :moderate
                                            :description "equipment-safety concern near lathe"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
  ...
  (testing "write-only-through-ledger: N settled operations -> N ledger facts"
    (let [[db actor] (fresh)]
      ...
      (exec-op actor "b" {:op :log-production-batch :effect :propose :subject "batch-001"
                          :patch {:product-spec :fabricated-spec}} coordinator)
